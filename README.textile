...
LJ is a base Rails application for ... that ... LJ is developed and maintained by GG

If you have any suggestions for improvements then feel free to leave a request.

h2. Install

* Download and unzip LJ
...
LJ comes with a few commonly used plugins installed and already setup.
...
LJ now comes with "Role Requirement":http://code.google.com/p/rolerequirement/ by Tim Harper. A default 
...
LJ, as of 0.3, has Open ID integrated with RESTful Authentication. Rejoice!

h3. Will Paginate

We use will_paginate in pretty much every project we use, so LJ comes with it pre-installed.

h3. Rspec & Rspec-rails

You should be testing your code, so LJ comes with Rspec and Rspec-rails already installed so you're
ready to roll.
...
LJ comes ready to rock capistrano. The recipe that is setup is based on using git and passenger. It's ready
...
LJ is setup to use the database to store sessions by default. 
...
LJ is using Bort put together by people at "Fudge":http://www.fudgestudios.com/ 
